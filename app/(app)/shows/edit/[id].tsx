...
export default function EditShow() {
  ...
  // Pre-populate show fields
  useEffect(() => {
    if (show) {
      setName(show.name ?? '');
      setShowDate(show.showDate ?? '');
      setShowTime(show.showTime ?? '');
      setLoadInTime(show.loadInTime ?? '');
      setSoundcheckTime(show.soundcheckTime ?? '');
      setDoorsTime(show.doorsTime ?? '');
      setSetLength(show.setLengthMinutes ? String(show.setLengthMinutes) : '');
      setNotes(show.notes ?? '');
    }
  }, [show]);

  // Pre-populate venue from existing link (runs once when venueStatus first loads)
  useEffect(() => {
    if (venueInitialized) return;
    if (venueStatus === undefined) return; // still loading
    if (venueStatus && venueStatus.venueName) {
      setSelectedVenue({
        _id: venueStatus.venueProfileId as string,
        name: venueStatus.venueName,
        city: venueStatus.venueCity ?? null,
        capacity: null,
      });
      setVenueSearch(venueStatus.venueName);
    }
    setVenueInitialized(true);
  }, [venueStatus, venueInitialized]);
  ...
  return (
    <ScreenContainer>
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingTop: 16, borderBottomWidth: 1, borderBottomColor: Colors.border }}>
        ...
        <Text style={{ fontFamily: 'DMSans_700Bold', fontSize: 18, color: Colors.textPrimary, flex: 1 }}>Edit Show</Text>
